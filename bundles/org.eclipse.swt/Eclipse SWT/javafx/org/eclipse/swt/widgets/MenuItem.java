...
public class MenuItem extends Item {
	...
	Menu parent;
	javafx.scene.control.MenuItem nativeItem;
	private Menu menu;
	...
	private void armInit() {
		EventHandler<javafx.event.Event> eventHandler = new EventHandler<javafx.event.Event>() {
			...
			public void handle(javafx.event.Event event) {
				...
				initDone = true;
				final Node node = nativeItem.impl_styleableGetNode();
				if( node != null ) {
					node.focusedProperty().addListener(new InvalidationListener() {
						@Override
						public void invalidated(Observable observable) {
							if( node.isFocused() ) {
								sendEvent(SWT.Arm, new Event(), true);	
							}
						}
					});
				}
		...
		if (parent.menu != null) {
			parent.menu.addEventHandler(javafx.scene.control.Menu.ON_SHOWN, eventHandler);			
		} else if (parent.contextMenu != null) {
			parent.contextMenu.addEventHandler(javafx.scene.control.Menu.ON_SHOWN,eventHandler);
		}
	}
...
//			case SWT.INSERT: return ??;
	...
	void registerHandle() {
		...
		if ((style & SWT.RADIO) != SWT.RADIO ) {
			nativeItem.setOnAction(new EventHandler<ActionEvent>() {
				@Override
